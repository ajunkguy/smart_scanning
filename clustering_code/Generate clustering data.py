...
class IP_Pool():
    ...
    def build_dhcp_device_matricx(self,dhcp_cutpoints):
        ...
        print('rand_classes',rand_classes)
        ...
        for i in range(len(dhcp_cutpoints)):
            ...
            print('rand_classes2',rand_classes2)
    ...
    def world_step(self):
        ...
        for i in range(len(self.dhcp_pool_list)): 
            #print(i,len(self.dhcp_pool_list[i].devices))
            #os.exit()
            '''
...
def get_lambda(scan_list,isc,upper_c = True):
    ...
    upper_confidence = 10/(scan_weight_class_y7[isc]+scan_weight_class_n7[isc]+1)
...
def start_test():
    ...
    for testip in range(ipnum):
        last_world_time7[testip] = world_time
    ...
    d_change_mat = np.zeros((pool_num,device_class_num+1))
    ...
    for i in range(int(60*24*30*12*10/ac)):
        ...
        while(frequency_sum1>1):
            ...
            #if ip_memory[target] == device_class_num and result != ip_memory[target]:#聚类用
            #if ip_memory[target] == device_class_num and result != ip_memory[target]:#聚类用
            d_change_mat[int(target/ip_pool_min_range),result] += 1
                #print(int(target/ip_pool_min_range))
        ...
        for testip in range(ipnum):
            ...
            if int(type5) == device_class_num:
                ...
                if ((t_24-c_time)% (24*60) < (24*60)*on_rate):#处于开机状态
                    ...
                    '''
                    print(ip_pool_1.test_one_ip(testip),ip_memory5[testip])
                    if (ip_pool_1.test_one_ip(testip)==ip_memory5[testip]):
                        print('?????????????????????????????')
                    '''
        ...
        for testip in range(ipnum):
            ...
            predict_outmap7[testip] = cacul_sum_p(last_world_time7[target],world_time,device_lambda_list7[type7])
        ...
        while(frequency_sum7>1):
            frequency_sum7 -= 1        
            target = np.argmax(predict_outmap7)
            result = ip_pool_1.test_one_ip(target)

            #扫描计数
            cishu7 += 1
            if result != ip_memory7[target]:
                value7_sumall+=1
            
            #记录扫描历史
            one_scan = (last_world_time7[target],world_time,result != ip_memory7[target])
            scan_device_list7[ip_memory7[target]].append(one_scan)
            
            
            #更新一些扫描结果变量
            ip_memory7[target] = result
            last_world_time7[target] = world_time
            predict_outmap7[target] = 0
        '''
        ...
        if i%test_time_range == test_time_range-1:  
            ...
            print('value123',value1_sumall-value1_sumall_mem,
                  value5_sumall-value5_sumall_mem,
            ...
            #print('cishu:',cishu1,cishu5)
            
            #print(d_change_mat)
            out_d_change_mat = copy.deepcopy(d_change_mat)
            for i_out_d in range(out_d_change_mat.shape[0]):
                out_d_change_mat[i_out_d,:] = out_d_change_mat[i_out_d,:]/sum(out_d_change_mat[i_out_d,:])
            #print(out_d_change_mat)
            ...
            #print(i/test_time_range)
            if i/test_time_range>=100:
                return out_d_change_mat
...
np.savetxt('out_mat.csv', out_mat, delimiter = ',') 
...
style.use('ggplot')
#生成演示用样本数据
data1 = np.random.normal(0,0.3,(100,2))
data2 = np.random.normal(1,0.2,(100,2))
data3 = np.random.normal(2,0.3,(100,2))

data = out_mat #np.concatenate((data1,data2,data3))

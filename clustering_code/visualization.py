# -*- coding: utf-8 -*-
"""
Created on Sat May 16 16:40:34 2020
...
df = pd.read_excel("test2.xlsx",index_col=0)  #index_col=0指定数据中第一列是类别名称，PS：计算机程序一般从整数0开始计数，所以0就代表第一列
#df = df.T    #python默认每行是一个样本，如果数据每列是一个样本的话，转置一下即可


Z = hierarchy.linkage(df, method ='single',metric='euclidean')
fig = plt.figure(figsize=(14, 4))
hierarchy.dendrogram(Z,labels = df.index,color_threshold = 0.129)
plt.xlabel('Address range ID')
plt.ylabel('Distance')
plt.savefig('out64.pdf')
plt.show()



#%%
label = cluster.hierarchy.cut_tree(Z,height=0.1)
...
y_label = 'PCl(%s%%)' % round((pca.explained_variance_ratio_[1]*100.0),2)   #y轴标签字符串
